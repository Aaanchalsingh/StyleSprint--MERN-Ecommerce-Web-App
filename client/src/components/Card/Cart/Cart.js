...
const Cart = () => {
    ...
    const fetchCartItems = async () => {
        try {
            const response = await axios.get('http://localhost:6969/api/cart/items');
                            ...
                            {cartItems.length > 0 &&
                                <div className="options">
                                    <div className="total__amount">
                                        ...
                                        <div className="total__amount__value">${cartItems.reduce((total, item) => total + item.price, 0)}</div>
                                    </div>
